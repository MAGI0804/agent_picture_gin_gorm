...
| 指南章节 | 要求 | 当前状态 | 已完成 | 未完成 / 风险 |
| --- | --- | --- | --- | --- |
| 12 前后端 API 建议 | run events、artifact 操作、memory 操作 | 已完成第一版 | `/api/v2/conversations/:id/runs` 已执行真实 workflow；`/api/v2/conversations/:id/runs/async` 已改为创建 run 后投递 Asynq 持久任务；新增 `POST /api/v2/runs/:id/resume`；已补 artifacts list、versions、preview、download、feedback、select、edit、render-text；memory search/delete/promote/patch 已有；前端已拆出 `agentV2/artifacts/memories` API client | 真正长连接增量推送未完成 |
| 16 前端体验建议 | 对话 + 产物 + 过程 + 版本 + 反馈 | 已完成第一版 | `/workspace` 支持输入、模型选择、运行、timeline、artifact board、版本、鉴权预览、下载、反馈、选择、Memory、Review/Eval、追问恢复、编辑、文字分层、失败重试、候选对比和刷新后恢复最近 run；Task 15 已拆组件和 composables | 更细的可视化 diff/并排大图对比可后续增强 |
| 21.10 前端工作台 | 记忆查看、候选图、版本、选择、下载、Review/Eval | 已完成第一版 | V2 工作台已支持候选图 rank 列表、推荐/选中标识、版本、下载、反馈、选择按钮、Memory 查看/删除/编辑/确认、Review/Eval 面板、编辑/重生成入口、候选对比和失败重试入口 | 组件视觉可继续按真实使用反馈细化 |
