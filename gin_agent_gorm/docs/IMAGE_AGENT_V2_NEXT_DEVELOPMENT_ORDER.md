...
- `task_ledger_items`、`tool_invocations` 表和 DAO 已有，但主链路尚未真正写入。
- 项目已有 Asynq/Redis 基础设施：`pkg/job`、`bootstrap/queue_job.go`、`job/foo_job.go`。

主要未完成：

- 前端 `/workspace` 真实模型手工冒烟仍待执行。
- 异步 Run 已接入 Asynq 持久队列第一版，worker 可抢占 `queued/failed -> running`；完整 step 幂等恢复、provider 错误分类和 running 超时扫描未做。
- `idempotency_key` 只有查询逻辑，没有数据库唯一约束。
- `task_ledger_items`、`tool_invocations` 未接入 runtime/tool 调用路径。
- `GET /runs/:id/events` 目前是一次性 SSE 输出已有 step，不是真正增量事件流。
- Requirement / Prompt 仍是规则型第一版，未调用文本 provider 生成结构化 prompt，也未做 schema 校验和追问恢复。
- 候选图 group 已有，但并行生成、逐张真实 review、ranker 精排未完成。
- OCR、版面检测、中文文字可读性检查未完成。
- Refiner Agent、自动二次生成、retry budget、版本 parent 链路未闭环。
- Memory 只有同 scope proposal 合并和自动晋级，语义去重、稳定记忆冲突降权、ranker 未完成。
...
说明：文档要求需求不足时进入 `waiting_user`，当前 `NeedClarification` 字段存在，但 runtime 没有暂停、前端也没有恢复回答链路。
