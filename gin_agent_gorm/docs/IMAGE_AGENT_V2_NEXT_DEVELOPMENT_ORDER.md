...
- Task 14 已补齐上传扩展名/MIME/像素/大小校验、object key 随机化、SafetyProvider 前后置审查、日志正文脱敏和本地存储路径防护；签名 URL 仍按外链需求延后，当前继续保持鉴权 API 为唯一入口。
...
当前状态：已完成第一版（2026-05-28）。新增 `eval_cases/eval_runs` model、AutoMigrate 和 DAO；新增 `EvolutionService`，支持最近 reflection 失败原因 Top 5 聚合、从 action item 生成 prompt version draft、`draft -> review -> active -> archived` 状态流转，并在激活新版本时归档同 agent 的旧 active 版本；新增 `/api/v2/evolution/*` 和 `/api/v2/eval/*` API；前端 `/workspace` 增加轻量 Evolution 面板，可查看失败聚合、生成 draft、review/active/archive prompt version。当前 eval run 先记录结果，不自动执行真实评测集。
...
当前状态：已完成第一版（2026-05-28）。新增 `internal/service/agent_v2/security` 本地安全策略，`tools.KindSafety` 已有静态 SafetyProvider 适配和 tool invocation 埋点；workflow 在 `prompt_agent` 后执行生成前文本安全审查，在 `image_generation_agent` 后执行生成后图片安全审查，手动 upload/edit 也会走安全检查。上传校验补齐扩展名与 MIME 对齐，非法扩展、MIME mismatch、超大小、超像素和解码失败均拒绝；V2 upload/generate/edit/render 产物 object key 增加随机不可预测路径段，`LocalObjectStore` 增加路径穿越防护。access log 会跳过上传和二进制体，并对 token、API key、prompt/content/messages 等字段递归脱敏。外链分享未新增，当前继续只暴露鉴权 preview/download API，静态 `/artifacts` 默认关闭。
...
当前状态：已完成第一版（2026-05-28）。前端已拆出 V2 API client：`frontend/src/api/agentV2.ts`、`artifacts.ts`、`memories.ts`；拆出 composables：`useAgentRun`、`useRunEvents`、`useArtifacts`、`useMemories`；拆出工作台组件：`WorkspaceComposer`、`TimelinePanel`、`ArtifactBoard`、`VersionStrip`、`ReviewPanel`、`MemoryPanel`、`EditPanel`。`AgentWorkspaceV2View.vue` 现在主要负责页面级状态编排；UI 增加 empty/queued/running/waiting_user/failed/cancelled/completed 的可读状态、失败 run 重试入口、候选对比和刷新后从 localStorage 恢复最近 run。长错误、长文本和候选对比补了 overflow-wrap 与稳定网格约束。
...
当前状态：已完成第一版（2026-05-28）。前端根路径、登录成功、注册成功和设置页导航均默认进入 `/workspace`，旧 `/chat` 继续作为历史会话和旧 artifact 兼容入口。`internal/service/agent_svc.AgentService` 已标记 deprecated，明确新图片 Agent 能力继续进入 `agent_v2`。旧 artifact 读取未删除，旧 API 的 preview/download 兼容保留；V2 产物继续走 `artifact_versions` 和鉴权 API。README、前端 README、部署文档、Google 配置文档和 V2 进度文档已同步默认入口与旧链路收敛策略。
