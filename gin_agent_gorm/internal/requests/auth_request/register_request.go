...
// SendRegisterEmailVerifyCodeRequest 是注册页发送邮箱验证码的请求参数。
type SendRegisterEmailVerifyCodeRequest struct {
	Email string `form:"email" json:"email,omitempty" valid:"email"`
}

// SendRegisterEmailVerifyCode 校验发送注册邮箱验证码的请求参数。
func SendRegisterEmailVerifyCode(data interface{}, c *gin.Context) map[string][]string {
